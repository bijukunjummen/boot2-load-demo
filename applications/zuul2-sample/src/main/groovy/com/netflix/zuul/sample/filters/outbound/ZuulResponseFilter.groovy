    ...
    HttpResponseMessage apply(HttpResponseMessage response) {
        ...
        if (SEND_RESPONSE_HEADERS.get()) {
            ...
            if (statusCategory != null) {
                headers.set(ZuulHeaders.X_ZUUL_STATUS, statusCategory.name())
            }
            ...
            headers.set(ZuulHeaders.X_ZUUL_PROXY_ATTEMPTS, headerStr)

            headers.set(ZuulHeaders.X_ZUUL, "zuul")
            headers.set(ZuulHeaders.X_ZUUL_INSTANCE, System.getenv("EC2_INSTANCE_ID") ?: "unknown")
            headers.set(ZuulHeaders.CONNECTION, ZuulHeaders.KEEP_ALIVE)
            headers.set(ZuulHeaders.X_ZUUL_FILTER_EXECUTION_STATUS, context.getFilterExecutionSummary().toString())
            headers.set(ZuulHeaders.X_ORIGINATING_URL, response.getInboundRequest().reconstructURI())

            if (response.getStatus() >= 400 && context.getError() != null) {
                Throwable error = context.getError()
                headers.set(ZuulHeaders.X_ZUUL_ERROR_CAUSE,
